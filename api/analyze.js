...
async function fetchAndStoreImage(imageUrl, bucket, storagePath) {
  ...
  try {
    ...
    // Vérifier que c'est bien une image (pas une page HTML 403)
...
function extractClubLogo(html) {
  // TM club logo patterns (tmssl CDN)
  const patterns = [
    // Wappen (écusson) dans le header de la fiche joueur
  ...
  for (const re of patterns) {
    ...
    if (m) {
      ...
      return url.split('?')[0]; // enlever query params
    }
  }
  return null;
}
...
export default async function handler(req, res) {
  ...
  // ─── 1. Extraire l'ID joueur depuis l'URL ───
  const playerId = tmUrl.match(/spieler\/(\d+)/)?.[1];

  // ─── 2. Photo joueur : URL directe depuis l'ID TM (FIABLE ✅) ───
  // TM photos suivent ce pattern prévisible : img.transfermarkt.com/portrait/big/{id}.jpg
  const playerPhotoUrl = playerId
    ? `https://img.transfermarkt.com/portrait/big/${playerId}.jpg`
    : null;

  // ─── 3. Logo club : fetch page TM pour extraire l'URL du logo ───
