...
function ProfilPage({authData, onUpdate}) {
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase.from('clubs').update({ name: clubName.trim(), logo_url: logoUrl.trim() }).eq('id', authData.club.id);
      if (error) throw error;
      setOk('✅ Club mis à jour !');
      if (onUpdate) onUpdate({ ...authData.club, name: clubName.trim(), logo_url: logoUrl.trim() });
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,maxWidth:800}}>
        <div className="card">
          ...
          <div style={{marginBottom:14}}>
            <label className="auth-lbl">URL du logo</label>
...
function AuthPage({onLogin}) {
  ...
  const handleCreateClub = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // Create club
      const { data: club, error: cErr } = await supabase.from('clubs').insert({ name: clubName.trim(), code, logo_url: logoUrl.trim() }).select().single();
      if (cErr) throw cErr;
      // Update profile
  ...
  return (
    <div className="auth-bg">
      <div className="auth-box">
        ...
        {mode === 'create' && (
          <div className="auth-field">
            <label className="auth-lbl">🖼️ Logo du club (URL optionnelle)</label>
            <input className="auth-input" placeholder="https://... ou laisser vide" value={logoUrl||''} onChange={e=>setLogoUrl(e.target.value)}/>
          </div>
        )}
