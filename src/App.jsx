...
.filter-btn{padding:5px 12px;border-radius:20px;font-size:11px;font-weight:500;cursor:pointer;border:1px solid var(--border);background:transparent;color:var(--muted);transition:all .15px;font-family:'Inter',sans-serif}
...
async function aiAnalyze(tmUrl, onStep){
  ...
  return {...emptyPlayer(),...d,tmUrl,photoUrl:data.photoUrl||"",clubLogoUrl:data.clubLogoUrl||"",id:Date.now()};
}
  ...
  return (
    <div>
      <div className="fiche-header">
        {p.photoUrl
          ? <div style={{width:80,height:80,borderRadius:14,overflow:'hidden',flexShrink:0,border:'2px solid var(--border)',background:'var(--surface)',boxShadow:'0 4px 16px rgba(0,0,0,.4)'}}>
              <img src={p.photoUrl} alt={p.nom} style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'top'}}
        ...
        <div className="overlay" onClick={e=>e.target===e.currentTarget&&setConfirmSign(null)}>
          <div className="modal" style={{maxWidth:440,textAlign:'center'}}>
            ...
            <div style={{fontSize:13,color:'var(--muted)',marginBottom:20,lineHeight:1.6}}>
              ...
              Son statut passera à <strong style={{color:'var(--green)'}}>Sous contrat</strong>.
            </div>
      ...
      <div className="tbl-wrap">
        <table>
          ...
          <tbody>
            ...
            {filtered.map((p,i)=>(
              <tr key={p.id} className={rowClass} style={{cursor:"pointer"}}
                onClick={()=>onView(p)}>
                ...
                <td>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    {p.photoUrl
                      ...
                      : <div style={{width:30,height:30,borderRadius:8,background:'var(--border)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,flexShrink:0}}>{ROLE_E[p.role]||"⚽"}</div>
                    }
                    <strong>{p.nom}</strong>
                  </div>
                </td>
                  ...
                  <div style={{display:"flex",gap:4}} onClick={e=>e.stopPropagation()}>
                    {isCibles && p.statut!=="Signé" && p.statut!=="Refusé" && onSign && (
                      <button className="btn btn-sm"
                        ...
                        title="Signer → Effectif"
                        onClick={btn(()=>setConfirmSign(p))}>✅</button>
                    )}
                    {isCibles && p.statut!=="Signé" && p.statut!=="Refusé" && onRefuse && (
                      <button className="btn btn-sm"
                        ...
                        onClick={btn(()=>onRefuse(p))}>❌</button>
                    )}
                    <button className="btn btn-ghost btn-sm" onClick={btn(()=>onEdit(p))}>✏️</button>
                    <button className="btn btn-red btn-sm" onClick={btn(()=>onDelete(p.id))}>✕</button>
                  </div>
                </td>
              </tr>
            ))}
...
function ComparateurPage({players}){
  ...
  return (
    <div>
      ...
      {(!pA||!pB)&&(
        <div className="info"><span>💡</span><div>Sélectionne deux joueurs pour les comparer côte à côte.</div></div>
      )}
    </div>
  );
}
...
══════════════════════════════════════════════════════════ */
const FORMATIONS = {
  "4-3-3": {label:"4-3-3",color:"#f5a623",positions:[{id:"GK",label:"G",x:50,y:8},{id:"RB",label:"DD",x:18,y:22},{id:"CB1",label:"DC",x:36,y:22},{id:"CB2",label:"DC",x:64,y:22},{id:"LB",label:"DG",x:82,y:22},{id:"CM1",label:"MC",x:28,y:44},{id:"CM2",label:"MC",x:50,y:44},{id:"CM3",label:"MC",x:72,y:44},{id:"RW",label:"AD",x:20,y:72},{id:"ST",label:"AC",x:50,y:78},{id:"LW",label:"AG",x:80,y:72}]},
  "4-4-2": {label:"4-4-2",color:"#3b82f6",positions:[{id:"GK",label:"G",x:50,y:8},{id:"RB",label:"DD",x:18,y:22},{id:"CB1",label:"DC",x:36,y:22},{id:"CB2",label:"DC",x:64,y:22},{id:"LB",label:"DG",x:82,y:22},{id:"RM",label:"MD",x:18,y:50},{id:"CM1",label:"MC",x:38,y:50},{id:"CM2",label:"MC",x:62,y:50},{id:"LM",label:"MG",x:82,y:50},{id:"ST1",label:"AC",x:35,y:76},{id:"ST2",label:"AC",x:65,y:76}]},
  "4-2-3-1": {label:"4-2-3-1",color:"#22c55e",positions:[{id:"GK",label:"G",x:50,y:8},{id:"RB",label:"DD",x:18,y:20},{id:"CB1",label:"DC",x:36,y:20},{id:"CB2",label:"DC",x:64,y:20},{id:"LB",label:"DG",x:82,y:20},{id:"CDM1",label:"MD",x:37,y:38},{id:"CDM2",label:"MD",x:63,y:38},{id:"RAM",label:"MOD",x:20,y:57},{id:"CAM",label:"MOC",x:50,y:57},{id:"LAM",label:"MOG",x:80,y:57},{id:"ST",label:"AC",x:50,y:78}]},
  "3-5-2": {label:"3-5-2",color:"#a855f7",positions:[{id:"GK",label:"G",x:50,y:8},{id:"CB1",label:"DC",x:28,y:22},{id:"CB2",label:"DC",x:50,y:22},{id:"CB3",label:"DC",x:72,y:22},{id:"RWB",label:"PLD",x:12,y:46},{id:"CM1",label:"MC",x:32,y:46},{id:"CM2",label:"MC",x:50,y:46},{id:"CM3",label:"MC",x:68,y:46},{id:"LWB",label:"PLG",x:88,y:46},{id:"ST1",label:"AC",x:35,y:76},{id:"ST2",label:"AC",x:65,y:76}]},
  "4-1-4-1": {label:"4-1-4-1",color:"#06b6d4",positions:[{id:"GK",label:"G",x:50,y:8},{id:"RB",label:"DD",x:18,y:20},{id:"CB1",label:"DC",x:36,y:20},{id:"CB2",label:"DC",x:64,y:20},{id:"LB",label:"DG",x:82,y:20},{id:"CDM",label:"MD",x:50,y:36},{id:"RM",label:"MD",x:16,y:55},{id:"CM1",label:"MC",x:36,y:55},{id:"CM2",label:"MC",x:64,y:55},{id:"LM",label:"MG",x:84,y:55},{id:"ST",label:"AC",x:50,y:78}]},
  "3-4-3": {label:"3-4-3",color:"#ef4444",positions:[{id:"GK",label:"G",x:50,y:8},{id:"CB1",label:"DC",x:28,y:22},{id:"CB2",label:"DC",x:50,y:22},{id:"CB3",label:"DC",x:72,y:22},{id:"RM",label:"MD",x:18,y:46},{id:"CM1",label:"MC",x:38,y:46},{id:"CM2",label:"MC",x:62,y:46},{id:"LM",label:"MG",x:82,y:46},{id:"RW",label:"AD",x:20,y:74},{id:"ST",label:"AC",x:50,y:80},{id:"LW",label:"AG",x:80,y:74}]},
  "5-3-2": {label:"5-3-2",color:"#f97316",positions:[{id:"GK",label:"G",x:50,y:8},{id:"RWB",label:"PLD",x:12,y:22},{id:"CB1",label:"DC",x:28,y:22},{id:"CB2",label:"DC",x:50,y:22},{id:"CB3",label:"DC",x:72,y:22},{id:"LWB",label:"PLG",x:88,y:22},{id:"CM1",label:"MC",x:28,y:50},{id:"CM2",label:"MC",x:50,y:50},{id:"CM3",label:"MC",x:72,y:50},{id:"ST1",label:"AC",x:35,y:78},{id:"ST2",label:"AC",x:65,y:78}]},
  "4-3-2-1": {label:"4-3-2-1",color:"#60a5fa",positions:[{id:"GK",label:"G",x:50,y:8},{id:"RB",label:"DD",x:18,y:20},{id:"CB1",label:"DC",x:36,y:20},{id:"CB2",label:"DC",x:64,y:20},{id:"LB",label:"DG",x:82,y:20},{id:"CM1",label:"MC",x:28,y:40},{id:"CM2",label:"MC",x:50,y:40},{id:"CM3",label:"MC",x:72,y:40},{id:"SS1",label:"SA",x:36,y:60},{id:"SS2",label:"SA",x:64,y:60},{id:"ST",label:"AC",x:50,y:80}]},
};
...
function PitchSVG({formation, lineup, onSlotClick, onSlotDrop}){
  ...
  if(!form) return null;
  const W=480, H=680;
  const handleDrop=(e,pos)=>{e.preventDefault();const pid=e.dataTransfer.getData("playerId");if(pid)onSlotDrop(pos.id,pid);};
  const handleDragOver=(e)=>e.preventDefault();
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",maxWidth:480,display:"block",borderRadius:12,boxShadow:"0 8px 40px rgba(0,0,0,.5)"}} xmlns="http://www.w3.org/2000/svg">
      <defs>
        <radialGradient id="pg2" cx="50%" cy="50%" r="60%"><stop offset="0%" stopColor="#16a34a" stopOpacity=".85"/><stop offset="100%" stopColor="#14532d" stopOpacity=".95"/></radialGradient>
      ...
      <rect width={W} height={H} fill="url(#pg2)" rx={12}/>
      {[0,1,2,3,4,5,6,7].map(i=>(<rect key={i} x={0} y={i*(H/8)} width={W} height={H/8} fill="rgba(0,0,0,.06)" opacity={i%2===0?1:0}/>))}
      ...
      {[[28,28],[W-28,28],[28,H-28],[W-28,H-28]].map(([cx,cy],i)=>(<circle key={i} cx={cx} cy={cy} r={7} fill="none" stroke="rgba(255,255,255,.2)" strokeWidth={1}/>))}
      {form.positions.map(pos=>{
        const px=pos.x/100*W, py=(1-pos.y/100)*H;
        ...
        return (
          <g key={pos.id} style={{cursor:"pointer"}} onClick={()=>onSlotClick(pos.id)} onDrop={(e)=>handleDrop(e,pos)} onDragOver={handleDragOver}>
            <ellipse cx={px} cy={py+28} rx={20} ry={5} fill="rgba(0,0,0,.3)"/>
            <circle cx={px} cy={py} r={22} fill={hasPlayer?ROLE_COLOR[role]:"rgba(0,0,0,.3)"} stroke={hasPlayer?ROLE_BORDER[role]:"rgba(255,255,255,.25)"} strokeWidth={hasPlayer?2:1.5} strokeDasharray={hasPlayer?"none":"4,3"} filter={hasPlayer?"url(#glow2)":"none"}/>
            {hasPlayer?(
              <>
                <text x={px} y={py-4} textAnchor="middle" dominantBaseline="middle" fill={ROLE_BORDER[role]} fontSize={9} fontFamily="'Bebas Neue',sans-serif" letterSpacing={0.5}>{ROLE_E[role]||"⚽"}</text>
                <text x={px} y={py+7} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize={7.5} fontFamily="Inter,sans-serif" fontWeight={700}>{player.nom.split(" ").pop().substring(0,8)}</text>
              </>
            ):(
              <text x={px} y={py} textAnchor="middle" dominantBaseline="middle" fill="rgba(255,255,255,.4)" fontSize={10} fontFamily="'Bebas Neue',sans-serif" letterSpacing={0.5}>{pos.label}</text>
            )}
            {hasPlayer&&(<text x={px} y={py+30} textAnchor="middle" fill="rgba(255,255,255,.85)" fontSize={8.5} fontFamily="Inter,sans-serif" fontWeight={600}>{player.nom.length>12?player.nom.substring(0,11)+".":player.nom}</text>)}
          </g>
        );
      })}
    </svg>
...
function TerrainPage({players}){
  ...
  const handleSlotClick=(slotId)=>{if(lineup[slotId]){setLineup(l=>{const n={...l};delete n[slotId];return n;});}else{setSelectSlot(slotId);}};
  const assignPlayer=(slotId,player)=>{const cleaned=Object.fromEntries(Object.entries(lineup).filter(([,v])=>v?.id!==player.id));setLineup({...cleaned,[slotId]:player});setSelectSlot(null);};
  const handleDrop=(slotId,playerId)=>{const player=players.find(p=>p.id==playerId);if(player)assignPlayer(slotId,player);};
  const clearAll=()=>{setLineup({});setSelectSlot(null);};
  const autoFill=()=>{
    const form=FORMATIONS[formation];const newLineup={};
    const roleMap={GK:["GK"],RB:["DEF"],LB:["DEF"],CB1:["DEF"],CB2:["DEF"],CB3:["DEF"],RWB:["DEF","MIL"],LWB:["DEF","MIL"],CDM:["MIL"],CDM1:["MIL"],CDM2:["MIL"],CM1:["MIL"],CM2:["MIL"],CM3:["MIL"],RM:["MIL"],LM:["MIL"],RAM:["MIL","ATT"],CAM:["MIL","ATT"],LAM:["MIL","ATT"],SS1:["ATT"],SS2:["ATT"],RW:["ATT"],LW:["ATT"],ST:["ATT"],ST1:["ATT"],ST2:["ATT"]};
    let pool=[...effectif];
    form.positions.forEach(pos=>{const roles=roleMap[pos.id]||["ATT"];const match=pool.find(p=>roles.includes(p.role));if(match){newLineup[pos.id]=match;pool=pool.filter(p=>p.id!==match.id);}else{const any=pool[0];if(any){newLineup[pos.id]=any;pool=pool.filter(p=>p.id!==any.id);}}});
    setLineup(newLineup);
  };
  const saveScheme=()=>{setSaved(true);setTimeout(()=>setSaved(false),2200);};
  ...
  return (
    <div>
      <div className="page-header"><div className="page-title">🏟️ TERRAIN <span>TACTIQUE</span></div><div className="page-sub">Positionnez vos joueurs, simulez vos systèmes de jeu</div></div>
      <div className="card" style={{marginBottom:16}}>
        <div className="section-title">SYSTÈME DE JEU</div>
        <div className="formation-selector">
          {Object.keys(FORMATIONS).map(f=>(<button key={f} className={`formation-btn ${formation===f?"active":""}`} onClick={()=>{setFormation(f);setLineup({});setSelectSlot(null);}} style={formation===f?{background:FORMATIONS[f].color,borderColor:FORMATIONS[f].color,color:"#07101f"}:{}}>{f}</button>))}
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginTop:8}}>
          ...
          <button className="btn btn-blue btn-sm" onClick={autoFill}>⚡ Auto-remplir</button>
        ...
        <div style={{marginTop:10,height:4,background:"var(--border)",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${FORMATIONS[formation].color},${FORMATIONS[formation].color}99)`,borderRadius:2,transition:"width .3s"}}/></div>
      </div>
      <div className="pitch-container">
        <div style={{flex:"1",minWidth:280,maxWidth:480}}>
          <PitchSVG formation={formation} lineup={lineup} onSlotClick={handleSlotClick} onSlotDrop={handleDrop}/>
          <div style={{display:"flex",gap:10,marginTop:10,flexWrap:"wrap",justifyContent:"center"}}>
            {[["GK","#60a5fa","#1e3a5f"],["DEF","#4ade80","#14532d"],["MIL","#93c5fd","#1e3a6e"],["ATT","#fca5a5","#7f1d1d"]].map(([r,c,bg])=>(<div key={r} style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"var(--muted)"}}><div style={{width:10,height:10,borderRadius:"50%",background:bg,border:`1.5px solid ${c}`}}/>{r}</div>))}
        ...
        <div className="pitch-sidebar">
          {selectSlot&&(
            <div className="card" style={{border:"1px solid var(--gold)",boxShadow:"0 0 0 2px rgba(245,166,35,.1)"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <div style={{fontSize:11,fontWeight:700,color:"var(--gold)",letterSpacing:.5}}>📍 {FORMATIONS[formation]?.positions.find(p=>p.id===selectSlot)?.label} — Choisir</div>
                <button className="btn btn-ghost btn-sm" onClick={()=>setSelectSlot(null)} style={{padding:"2px 7px"}}>✕</button>
              </div>
              {available.length===0&&<div style={{fontSize:11,color:"var(--dim)",fontStyle:"italic"}}>Tous les joueurs sont placés</div>}
              <div style={{display:"flex",flexDirection:"column",gap:5,maxHeight:280,overflowY:"auto"}}>
                {available.map(p=>(<div key={p.id} className="pitch-player-card" onClick={()=>assignPlayer(selectSlot,p)}><div className="pitch-player-card-name">{p.nom}</div><div className="pitch-player-card-pos">{ROLE_E[p.role]} {p.poste||p.role} {p.club?"· "+p.club:""}</div></div>))}
              </div>
            </div>
          )}
          {!selectSlot&&(
            <>
              <div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:5,maxHeight:360,overflowY:"auto"}}>
                  ...
                  {available.map(p=>(<div key={p.id} className="pitch-player-card" draggable onDragStart={e=>e.dataTransfer.setData("playerId",p.id)}><div className="pitch-player-card-name">{p.nom}</div><div className="pitch-player-card-pos">{ROLE_E[p.role]} {p.poste||p.role}</div></div>))}
                </div>
              </div>
              <div style={{marginTop:8}}>
                <div className="pitch-sidebar-title">SUR LE TERRAIN <span style={{color:"var(--green)"}}>{filledCount}</span></div>
                <div style={{display:"flex",flexDirection:"column",gap:5,maxHeight:280,overflowY:"auto"}}>
                  {Object.entries(lineup).filter(([,p])=>p).map(([slotId,p])=>(<div key={slotId} className="pitch-player-card" style={{borderColor:"rgba(34,197,94,.2)",background:"rgba(34,197,94,.04)",cursor:"pointer"}} onClick={()=>handleSlotClick(slotId)}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div className="pitch-player-card-name">{p.nom}</div><span style={{fontSize:9,color:"var(--red)",marginLeft:6}}>✕</span></div><div className="pitch-player-card-pos">{FORMATIONS[formation]?.positions.find(pos=>pos.id===slotId)?.label} · {p.poste||p.role}</div></div>))}
                </div>
              </div>
            </>
          )}
          <div style={{marginTop:8}}>
            ...
            <textarea className="form-textarea" rows={4} value={tacNote} onChange={e=>setTacNote(e.target.value)} placeholder="Principes de jeu, consignes, bloc..."/>
          </div>
          {effectif.length===0&&(<div className="warn" style={{marginTop:8}}><span>⚠️</span><div>Ajoute des joueurs dans l'onglet Effectif.</div></div>)}
        </div>
      </div>
    </div>
  );
...
function GuidePage(){
  ...
  const sections=[
    {title:"🔍 COMMENT RÉCUPÉRER LES DONNÉES D'UN JOUEUR",color:"var(--blue)",items:[["1. URL Transfermarkt","Va sur transfermarkt.fr → cherche le joueur → copie l'URL → colle dans l'onglet 'Analyser'. L'IA récupère automatiquement : valeur, contrat, agent, nationalité, stats."],["2. SofaScore","Pour les stats avancées (xG, xA, duels %, note) : SofaScore.com → copier l'URL dans la fiche joueur → section Stats."],["3. Vérification manuelle","Pour les joueurs non trouvés (erreur 403 sur TM) : compléter manuellement via le bouton ✏️ Modifier."],["4. Agents","Chercher l'agent sur LinkedIn. Vérifier sur FootMercato ou L'Équipe pour les rumeurs de transfert."]]},
    {title:"📋 WORKFLOW SCOUTING — STATUTS",color:"var(--green)",items:[["✅ Identifié","TM URL ajoutée · Poste confirmé · Âge + contrat vérifiés · Fiche créée dans l'app"],["✅ Contacté","Agent ou joueur contacté · Intérêt confirmé · Prétentions salariales approx. reçues"],["✅ Observé","Min. 3 matchs analysés (vidéo) · Fiche complète · Note DS attribuée · Points forts/faibles remplis"],["✅ En négociation","Offre formelle envoyée · Durée + salaire définis · Accord agent · Visite médicale planifiée"],["✅ Signé","Contrat signé · Enregistrement FFAndorre · Annonce officielle"]]},
    {title:"🏔️ RÈGLES SPÉCIFIQUES ANDORRE D2",color:"var(--purple)",items:[["Joueurs étrangers","Vérifier chaque saison le quota de joueurs non-andorrans autorisés en D2 auprès de la FFAndorre."],["Permis de travail","Tout joueur hors UE doit obtenir un permis de travail andorran. Délai : 4-8 semaines."],["Terrain synthétique","Majorité des terrains en Andorre sont synthétiques. Prioriser joueurs habitués."],["Altitude (1000-1500m)","Préparer une semaine d'acclimatation."],["Langue","Espagnol + catalan dans le vestiaire."]]},
    {title:"💡 TIPS MERCATO ÉTÉ 2026",color:"var(--gold)",items:[["Joueurs libres en priorité","Contacter dès janvier 2026 les joueurs dont le contrat expire en juin 2026 → zéro indemnité."],["Agents identifiés","Talent Sport (3 joueurs), Agencysports (1), Goalactic (1)."],["Réseau France","Cibler National 2 / National 3 : niveau adapté à D2 Andorre, salaires accessibles (800-1500€/mois)."],["Andorrans obligatoires","Inclure 2-3 joueurs andorrans dans l'effectif."]]},
  ];
  return (
    <div>
      ...
      {sections.map((s,i)=>(
        <div className="guide-section" key={i}>
          <div className="guide-hdr" style={{color:s.color}} onClick={()=>setOpen(open===i?-1:i)}>{s.title} <span>{open===i?"▲":"▼"}</span></div>
          {open===i&&<div className="guide-body">{s.items.map(([k,v],j)=>(<div className="guide-row" key={j}><div className="guide-key">{k}</div><div className="guide-val">{v}</div></div>))}</div>}
        </div>
      ))}
    </div>
  );
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:20,flexWrap:'wrap'}}>
          <div style={{width:90,height:90,borderRadius:12,background:'var(--bg)',border:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',flexShrink:0}}>
            {logoUrl?<img src={logoUrl} alt="logo" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:32}}>🏟️</span>}
          </div>
          <div>
      ...
      <div className="card">
        ...
        <div className="fiche-grid-2">
          {[['Nom',user.nom||'—'],['Rôle',user.role||'membre'],['Plan',club.plan||'free'],['Code club',club.code||'—']].map(([l,v])=>(
            <div className="fiche-field" key={l}><div className="fiche-label">{l}</div><div className="fiche-val" style={l==='Code club'?{fontFamily:'monospace',color:'var(--gold)',letterSpacing:2}:{}}>{v}</div></div>
          ))}
        </div>
      </div>
    </div>
...
function AdminPage({onLogout}){
  ...
  const loadData=async()=>{
    ...
    try{
      ...
      setClubs(clubsData||[]);setUsers(profilesData||[]);
  ...
  return (
    <div className="admin-bg">
      <div className="admin-header">
        <div><div className="admin-title">⚙️ SCOUTROOM ADMIN</div><div style={{fontSize:11,color:'var(--muted)'}}>Tableau de bord administrateur</div></div>
      ...
      {loading?<div style={{color:'var(--muted)',textAlign:'center',padding:40}}>Chargement...</div>:(
        <>
          <div className="admin-grid">
            {[[clubs.length,'🏟️ Clubs','var(--gold)'],[users.length,'👥 Utilisateurs','var(--blue)'],[users.filter(u=>u.club_id).length,'✅ Membres actifs','var(--green)'],[clubs.filter(c=>c.plan==='pro').length,'⭐ Clubs Pro','var(--purple)']].map(([n,l,c])=>(<div className="admin-kpi" key={l}><div className="admin-kpi-n" style={{color:c}}>{n}</div><div className="admin-kpi-l">{l}</div></div>))}
          </div>
          <div className="admin-section">
            ...
            {clubs.map(club=>(<div className="club-row" key={club.id}><div><div style={{fontWeight:600,marginBottom:2}}>{club.name}</div><div style={{fontSize:11,color:'var(--muted)'}}>{new Date(club.created_at).toLocaleDateString('fr-FR')} · {users.filter(u=>u.club_id===club.id).length} membre(s)</div></div><div style={{display:'flex',alignItems:'center',gap:10}}><span className="club-code">{club.code}</span><span className={`tag ${club.plan==='pro'?'tag-purple':'tag-gray'}`}>{club.plan}</span><button className="btn btn-red btn-sm" onClick={()=>deleteClub(club.id)}>🗑️</button></div></div>))}
          </div>
          <div className="admin-section">
            ...
            <div className="tbl-wrap">
              <table>
                ...
                <tbody>{users.map(u=>(<tr key={u.id}><td style={{fontWeight:500}}>{u.nom||'—'}</td><td style={{color:'var(--muted)'}}>{u.clubs?.name||'—'}</td><td><span className={`tag ${u.role==='admin'?'tag-gold':u.role==='ds'?'tag-blue':'tag-gray'}`}>{u.role||'membre'}</span></td><td style={{color:'var(--muted)',fontSize:11}}>{new Date(u.created_at).toLocaleDateString('fr-FR')}</td></tr>))}</tbody>
              </table>
            </div>
          </div>
        </>
  ...
  return (
    <div className="auth-bg">
      <div className="auth-box">
        ...
        {mode!=='login'&&(<div className="auth-field"><label className="auth-lbl">Prénom / Nom</label><input className="auth-input" placeholder="Thomas Martin" value={nom} onChange={e=>setNom(e.target.value)}/></div>)}
        <div className="auth-field"><label className="auth-lbl">Email</label><input className="auth-input" type="email" placeholder="ton@email.com" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()}/></div>
        <div className="auth-field"><label className="auth-lbl">Mot de passe</label><input className="auth-input" type="password" placeholder="••••••••" value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()}/></div>
        {mode==='join'&&(<div className="auth-field"><label className="auth-lbl">🔑 Code club</label><input className="auth-input auth-code" placeholder="ex: ANDORRE26" value={clubCode} onChange={e=>setClubCode(e.target.value)}/></div>)}
        {mode==='create'&&(<div className="auth-field"><label className="auth-lbl">🏟️ Nom du club</label><input className="auth-input" placeholder="ex: FC Andorra B" value={clubName} onChange={e=>setClubName(e.target.value)}/></div>)}
        <button className="auth-btn" onClick={submit} disabled={loading}>{loading?'⏳ En cours...':mode==='login'?'→ Se connecter':mode==='join'?'→ Rejoindre le club':'→ Créer mon club'}</button>
        {mode==='login'&&(<div style={{textAlign:'center',marginTop:12}}><button style={{background:'none',border:'none',color:'var(--muted)',fontSize:11,cursor:'pointer',textDecoration:'underline'}} onClick={async()=>{if(!email.trim()){setErr("Entre ton email d'abord.");return;}await supabase.auth.resetPasswordForEmail(email.trim(),{redirectTo:'https://scoutroom.vercel.app/?reset=true'});setErr('');alert('📧 Email de réinitialisation envoyé !');}}>Mot de passe oublié ?</button></div>)}
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(async({data:{session}})=>{
      if(session?.user){
        ...
        if(profile?.club_id){setAuthData({user:session.user,profile,club:profile.clubs});}
      }
  ...
  const handleLogout=async()=>{await supabase.auth.signOut();setAuthData(null);setPlayers([]);};
  ...
  const handleAnalyze=async()=>{
    if(!url.trim())return;
    setAnalyzeError("");setSaveError("");setPreview(null);setLoading(true);
    try{const p=await aiAnalyze(url.trim(),setLoadStep);setPreview(p);}
    catch(e){setAnalyzeError(`Erreur analyse: ${e.message}`);}
    ...
    <>
      ...
      {newClubCode&&(
        <div style={{background:'rgba(34,197,94,.12)',border:'1px solid rgba(34,197,94,.25)',borderRadius:10,padding:'12px 20px',margin:'16px',display:'flex',alignItems:'center',gap:12,justifyContent:'space-between'}}>
          <div>🎉 <strong>Club créé !</strong> Ton code d'invitation : <span style={{fontFamily:'monospace',fontWeight:900,fontSize:16,color:'var(--green)',letterSpacing:2}}>{newClubCode}</span></div>
      ...
      <div className="app">
        <div className="topbar">
          <div className="logo-wrap">
            <div className="logo-icon">{clubLogo?<img src={clubLogo} alt="logo" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:10}}/>:"SR"}</div>
            <div><div className="logo">SCOUT<span>ROOM</span></div><div className="logo-club">{clubName}</div></div>
          </div>
          <div className="tabs">
            {TABS.map(t=>(<button key={t.id} className={`tab ${tab===t.id?"active":""}`} onClick={()=>{window.location.hash=t.id;setTab(t.id);}}>{t.label}{t.count!=null&&<span className="tab-badge">{t.count}</span>}</button>))}
          </div>
          <div className="topbar-right">
            ...
            <button className="btn btn-ghost btn-sm" onClick={()=>exportCSV(players)}>📥 CSV</button>
            {authData?.user?.email===ADMIN_EMAIL&&<button className="btn btn-ghost btn-sm" style={{color:"var(--gold)",borderColor:"rgba(245,166,35,.3)"}} onClick={()=>{window.location.hash="admin";setTab("admin");}}>⚙️ FM</button>}
            <div className="user-pill"><span style={{fontSize:14}}>👤</span><span>{user?.nom||"Utilisateur"}</span></div>
        ...
        <div className="main">
              ...
              {(alertRed.length>0||alertOrange.length>0)&&(
                <div className="alert-banner">
                  ...
                  <div className="alert-list">
                    {alertRed.map(p=>(<span key={p.id} className="alert-pill" onClick={()=>setViewP(p)}>🔴 {p.nom} — exp. {p.finContrat}</span>))}
                    {alertOrange.map(p=>(<span key={p.id} className="alert-pill alert-pill-orange" onClick={()=>setViewP(p)}>🟠 {p.nom} — exp. {p.finContrat}</span>))}
                  </div>
                </div>
              )}
              <div className="kpi-grid">
                <div className="kpi" style={{borderTop:"2px solid var(--blue)"}}><div className="kpi-n" style={{color:"var(--blue)"}}>{effectif.length}</div><div className="kpi-l">👥 Effectif actuel</div><div className="kpi-icon">👥</div></div>
                  ...
                  <div className="kanban">
                    {[["Identifié","var(--muted)"],["Contacté","var(--blue)"],["Observé","var(--purple)"],["En négociation","var(--orange)"],["Signé","var(--green)"],["Refusé","var(--red)"]].map(([s,c])=>(
                      <div className="kol" key={s}>
                        ...
                        {cibles.filter(p=>p.statut===s).map(p=>(<div className="kcard" key={p.id} style={{borderLeft:`2px solid ${c}`}} onClick={()=>setViewP(p)}><div className="kcard-name">{p.nom}</div><div className="kcard-pos">{p.poste} · {p.priorite}</div></div>))}
          ...
          {tab==="analyser"&&(
            <div>
              {!preview&&!loading&&(
                <div className="url-page">
                  <div className="url-hero">
                    ...
                    <p>L'IA récupère automatiquement toutes les données et remplit la fiche complète.</p>
                  </div>
                  ...
                  {analyzeError&&<div className="err"><span>⚠️</span><div>{analyzeError}</div></div>}
              ...
              {preview&&!loading&&(
                <div>
                  {saveError&&<div className="err"><span>⚠️</span><div>{saveError}</div></div>}
                  ...
                  <FicheView player={preview} onEdit={()=>setEditP(preview)} onSave={handleSavePreview} onDiscard={()=>{setPreview(null);setUrl("");setSaveError("");setAnalyzeError("");}} isSaved={false}/>
                </div>
              )}
            </div>
          )}
          ...
          <div className="overlay" onClick={e=>e.target===e.currentTarget&&setViewP(null)}>
            <div className="modal modal-lg">
              ...
              <div className="modal-actions no-print">
                ...
                {viewP.categorie==="CIBLE"&&viewP.statut==="Signé"&&(<span style={{fontSize:12,color:'var(--green)',fontWeight:600}}>✅ Joueur signé · dans l'Effectif</span>)}
                {viewP.categorie==="CIBLE"&&viewP.statut==="Refusé"&&(<span style={{fontSize:12,color:'var(--red)',fontWeight:600}}>❌ Dossier refusé</span>)}
                <div style={{flex:1}}/>
