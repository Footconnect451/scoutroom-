...
@media(max-width:768px){
  ...
  /* Topbar — split into brand + bottom nav */
  .topbar{
    height:auto;flex-direction:column;align-items:stretch;
    padding:10px 14px 0;gap:0;position:sticky;top:0;z-index:200;
  }
  .topbar > div:first-child{padding-bottom:8px}
  .tabs{
  ...
  .tab{
    font-size:10px;padding:10px 10px;white-space:nowrap;
    flex-shrink:0;border-radius:0;border-bottom:2px solid transparent;
  }
  .tab.active{background:transparent;color:var(--gold);border-bottom-color:var(--gold)}
  ...
  .modal{padding:16px;border-radius:12px 12px 0 0;position:fixed;bottom:0;left:0;right:0;max-height:90vh;max-width:100%}
  ...
  /* Admin */
  .admin-grid{grid-template-columns:repeat(2,1fr)}
  .admin-bg{padding:14px}

  /* Guide */
  .guide-row{grid-template-columns:1fr;gap:0}
  .guide-key{border-right:none;border-bottom:1px solid rgba(28,46,71,.5)}
  ...
  /* Filter bar */
  .filter-bar{gap:4px}
  .filter-btn{padding:4px 9px;font-size:10px}
...
function ProfilPage({authData, onUpdate}) {
  ...
  return (
    <div>
      ...
      <div className='profil-grid' style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,maxWidth:800}}>
        <div className="card">
...
export default function App(){
  ...
  const getHashTab=()=>{const h=window.location.hash.replace('#','');const v=['dashboard','effectif','targets','analyze','budget','guide','profil','admin'];return v.includes(h)?h:'dashboard';};
  ...
  const handleSavePreview=async()=>{
    ...
    const dest=preview.categorie==="EFFECTIF"?"effectif":"targets";window.location.hash=dest;setTab(dest);
  };
